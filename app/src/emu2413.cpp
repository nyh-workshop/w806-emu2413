...
static preCalculateSincTable<int16_t, sizeof(int16_t) *SINC_RESO *LW / 2> sinc_tbl = preCalculateSincTable<int16_t, sizeof(int16_t) * SINC_RESO * LW / 2>(MSX_CLK / 72, DEFAULT_SAMPLE_RATE);
...
void OPLL_RateConv_new(OPLL_RateConv* conv, double f_inp, double f_out, int ch) {
  ...
  // C-Sky's compiler does not really like static constexpr declarations with other variables.
  // For the table, it must be fixed to the default MSX_CLK and the sample rate which is 44100.
  // f_out = opll->rate = DEFAULT_SAMPLE_RATE
  // f_inp = opll->clk / 72.0 = MSX_CLK / 72
  _emu2413_assert(f_inp == MSX_CLK / 72, "MSK_CLK must be 3579545!\n");
  _emu2413_assert(f_out == DEFAULT_SAMPLE_RATE, "Sample rate must be 44100!\n");

  // Make sure ch = 2!
  _emu2413_assert(conv->ch == 2, "conv->ch must be only 2 for now!\n");
...
{
  _emu2413_assert(rateConvPtr != nullptr, "RateConv is not initialized!\n");
